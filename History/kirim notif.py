                        ...
                        if name != "Gk Kenal":
                            ...
                            if duration >= 1.0:  # cukup 1 detik
                                if detect_blink(frame, name) and recent_blink(name):
                                    ...
                                    # Kirim notifikasi Telegram (sekali per hari)
                                    chat_id = get_chat_id(name)
                                    tanggal_hari_ini = datetime.now().strftime("%Y-%m-%d")
                                    key_telegram = f"{name}-{tanggal_hari_ini}"

                                    if chat_id and key_telegram not in telegram_dicatat_hari_ini:
                                        jam_obj = datetime.strptime(jam_datang, "%H:%M").time()
                                        status = "Tepat Waktu" if jam_obj <= JAM_TEPAT_WAKTU else "Terlambat"
                                        pesan_telegram = f"Halo, {name} telah melakukan absensi pada pukul {jam_datang}. Status: {status}."
                                        kirim_telegram(chat_id=chat_id, message=pesan_telegram)
                                        telegram_dicatat_hari_ini.add(key_telegram)

                                    # Simpan foto (sekali per hari)
                                    ...
                                    dicatat_hari_ini.add(f"{name}-{datetime.now().strftime('%a/%d')}")
                                    # Jangan hapus recognized_since di sini agar tidak reset!
                        else:
                            # Jika wajah tidak dikenali, reset recognized_since untuk orang itu
